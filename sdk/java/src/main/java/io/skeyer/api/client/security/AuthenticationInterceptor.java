package io.skeyer.api.client.security;
...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean isApiKeyRequired = original.header(SkeyerConstants.ENDPOINT_SECURITY_TYPE_APIKEY) != null;
        boolean isSignatureRequired = original.header(SkeyerConstants.ENDPOINT_SECURITY_TYPE_SIGNED) != null;
        newRequestBuilder.removeHeader(SkeyerConstants.ENDPOINT_SECURITY_TYPE_APIKEY)
                .removeHeader(SkeyerConstants.ENDPOINT_SECURITY_TYPE_SIGNED);

        // Endpoint requires sending a valid API-KEY
        if (isApiKeyRequired || isSignatureRequired) {
            newRequestBuilder.addHeader(SkeyerConstants.API_KEY_HEADER, apiKey);
        }
